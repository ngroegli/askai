...
print_error_or_warnings = shared_utils.print_error_or_warnings
...
class TestPrintUtilities(BaseUnitTest):
    ...
    def test_print_error_function(self):
        ...
        try:
            # Mock cprint to capture calls without output
            with patch('askai.utils.helpers.cprint') as mock_cprint:

                test_message = "Test error message"
                # Use exit_on_error=False to prevent sys.exit() during test
                print_error_or_warnings(test_message, exit_on_error=False)

                # Verify cprint was called
                self.assert_true(
                    mock_cprint.called,
                    "print_error_called",
                    "Error printing function was called"
                )
    ...
    def test_print_warning_function(self):
        ...
        try:
            with patch('askai.utils.helpers.cprint') as mock_cprint:
                ...
                # Test with different types of messages (but only strings work with print_error_or_warnings)
                test_cases = [
                ...
                for case in test_cases:
                    print_error_or_warnings(case, is_warning=True, exit_on_error=False)

                # Should have called cprint multiple times
                self.assert_true(
                    mock_cprint.call_count >= len(test_cases),
                    "print_warning_multiple",
                    "Handles multiple warning calls"
                )
    ...
    def test_colored_output(self):
        ...
        try:
            # Mock termcolor.cprint to prevent actual colored output
            with patch('askai.utils.helpers.cprint') as mock_cprint:
                mock_cprint.return_value = None

                # Try to import and test colored functions
                try:
                    print_error_or_warnings("Test colored message", exit_on_error=False)

                    # Verify cprint was called
                    self.assert_true(
                        mock_cprint.called,
                        "colored_output_called",
                        "Colored output function was called"
                    )
