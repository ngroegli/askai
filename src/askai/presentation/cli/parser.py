    ...
    def validate_arguments(self, args, logger):
        ...
        if args.openrouter is not None:
            if not args.openrouter:  # Empty sequence evaluates to False in boolean context
                ...
                print_error_or_warnings(
                    "--openrouter requires a command (check-credits, list-models)"
                )
                sys.exit(1)
            ...
            if command not in valid_commands:
                ...
                print_error_or_warnings(
                    f"Invalid OpenRouter command '{command}'. Valid commands: {', '.join(valid_commands)}"
                )
                sys.exit(1)
        ...
        if args.plain_md and args.format != "md":
            ...
            print_error_or_warnings(
                "--plain-md can only be used with -f md. The parameter --plain-md will be ignored.",
                is_warning=True, exit_on_error=False
            )
