...
from askai.utils import load_config, print_error_or_warnings
            ...
            if not chats and not corrupted_files:
                ...
                if corrupted_files:
                    print_error_or_warnings(
                        f"Found {len(corrupted_files)} corrupted chat files:",
        ...
        if choice == '1':
            ...
            for chat_id in corrupted_files:
                if self.chat_manager.repair_chat_file(chat_id):
                    ...
                    print_error_or_warnings(f"Failed to repair: {chat_id}", is_warning=True, exit_on_error=False)
            ...
            for chat_id in corrupted_files:
                if self.chat_manager.delete_chat(chat_id):
                    ...
                    print_error_or_warnings(f"Failed to delete: {chat_id}", is_warning=True, exit_on_error=False)
            ...
            while True:
                ...
                try:
                    ...
                    if 0 <= file_idx < len(corrupted_files):
                        ...
                        if action == '1':
                            if self.chat_manager.repair_chat_file(selected_chat_id):
                                ...
                                print_error_or_warnings(
                                    f"Failed to repair: {selected_chat_id}",
                                    is_warning=True, exit_on_error=False
                                )
                            ...
                            if confirm == 'y':
                                if self.chat_manager.delete_chat(selected_chat_id):
                                    ...
                                    print_error_or_warnings(
                                        f"Failed to delete: {selected_chat_id}",
                                        is_warning=True, exit_on_error=False
                                    )
                    else:
                        print_error_or_warnings(
                    ...
                    print_error_or_warnings("Please enter a valid number.", is_warning=True, exit_on_error=False)
        else:
